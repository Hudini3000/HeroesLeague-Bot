...
勇者联盟 - 首领自动挂机脚本 v8
===============================
从 v7 升级：加入凌晨 4 点自动重启功能（释放内存，更像人类操作）。
...
核心逻辑：
  ...
  6. 快速点击"前往"（多尺度）→ 等待战斗
  7. 【新增】凌晨4点自动退出，由watchdog自动重启
  8. 循环

使用方法：
  python boss_auto_v8.py
  热键: F2=暂停/继续  F4=停止
  诊断模式: python boss_auto_v8.py --diagnose

自动重启：
  1. 用 watchdog.bat 启动脚本（双击 watchdog.bat）
  2. 脚本在凌晨4点会自动退出
  3. watchdog.bat 检测到退出后，等待10秒自动重启
  4. 无限循环，实现"每天凌晨4点重启一次"
"""
import os
import sys
import time
...
# 【新增】检查是否需要自动重启
# ============================================================
def check_auto_restart(bot):
    """
    检查当前时间是否在自动重启时间窗口内。
    如果是，先回到主界面，然后退出脚本。
    返回: True=需要重启, False=不需要
    """
    now = time.localtime()
    restart_hour = CONFIG["restart_hour"]
    restart_window = CONFIG["restart_window"]

    # 检查是否在重启时间窗口内
    if now.tm_hour == restart_hour and now.tm_min < restart_window:
        # 避免重复重启：检查今天是否已经重启过了
        marker_file = os.path.join(os.path.dirname(__file__), ".restart_marker")
        today = time.strftime("%Y-%m-%d")

        if os.path.exists(marker_file):
            with open(marker_file, "r") as f:
                last_restart = f.read().strip()
            if last_restart == today:
                return False  # 今天已经重启过了

        # 标记今天已重启
        with open(marker_file, "w") as f:
            f.write(today)

        log(f"=== 到达自动重启时间 {restart_hour:02d}:00，准备重启 ===")
        return True
...
# 【新增】回到主界面（更像人类操作）
# ============================================================
def go_back_to_main(win, origin):
    """
    按ESC键回到主界面。
    模拟人类在关闭游戏前按ESC回到主菜单的行为。
    """
    log("  正在回到主界面...")
    for _ in range(3):
        pyautogui.press('escape')
        time.sleep(rand(0.5, 1.0))
    log("  已回到主界面，脚本将在3秒后退出")
    time.sleep(3)
...
class BossBot:
    def __init__(self):
        ...
        self.stopped = False
        self.total = 0
        self.restart_flag = False  # 标记是否需要重启
    ...
    def run(self):
        log("=== 勇者联盟首领挂机 v8 启动 ===")
        log(f"  自动重启时间: 每天凌晨 {CONFIG['restart_hour']:02d}:00-{CONFIG['restart_hour']:02d}:{CONFIG['restart_window']:02d}")
        log("热键: F2=暂停  F4=停止")
        check_templates()
        ...
        while not self.stopped:
            ...
            # 【新增】检查是否需要自动重启
            if check_auto_restart(self):
                self.restart_flag = True
                go_back_to_main(win, origin)
                break  # 退出主循环，脚本将在__main__里退出
        ...
        if self.restart_flag:
            log("=== 自动重启模式：脚本将退出，watchdog 会在10秒后重启 ===")
            sys.exit(0)  # 正常退出，让watchdog重启
